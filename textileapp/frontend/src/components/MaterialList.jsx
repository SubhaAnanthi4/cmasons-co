    ...
    try {
      ...
      const materialsWithExtra = await Promise.all(
        response.data.map(async (material) => {
          try {
            const batchRes = await axios.get(`http://localhost:5000/batch-returns/${material._id}`);
  ...
  const handleSubmitMaterial = async (e) => {
    ...
    try {
      if (editingMaterialId) {
        await axios.put(`http://localhost:5000/materials/${editingMaterialId}`, {
        ...
        if (newMaterial.batchQuantity && newMaterial.batchDate) {
          await axios.post('http://localhost:5000/batch-return', {
        ...
        const res = await axios.post('http://localhost:5000/materials', {
          materialName: newMaterial.materialName,
          toCompany: newMaterial.toCompany,
          givenQuantity: newMaterial.givenQuantity,
          dispatchDate: newMaterial.dispatchDate
